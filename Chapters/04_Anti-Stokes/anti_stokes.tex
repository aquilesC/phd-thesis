...
particle didn't reshape while being excited in resonance. 
...
obtained in this case is $293\pm 6 \K$, while room temperature was $296\K$. 

The accuracy of the obtained values depends to a great extent on the plasmon
resonance. The first step in the temperature extraction is the determination of
the term $I_\textrm{SPR}$ in equation \ref{eqn:fitting}. This is achieved by
fitting a lorentzian to the emission spectrum obtained while irradiating with a
$532\nm$ laser. In figure \ref{fig:spectra_rod} it is possible to observe that
the emission spectrum is not perfectly lorentzian and therefore the fitting
results will be sensitive to the portion of the spectrum selected. The asymmetry
in the luminescence is mainly due to a broadband emission arising from gold
between the excitation energy and the plasmon energy, as was discussed in the
introduction. Depending on the wavelength range selected, the parameters of the
lorentzian fit (mainly its width and peak position) can slightly change and this
will give rise to different extracted temperatures.
...
uncertainty induced by the plasmon resonance has to be reached depending on
each application. The error bar in figure \ref{fig:AS_in_Log} and in the
following figures is the result of the estimation of the temperature uncertainty
because of variations in the plasmon resonance.
...
respectively. It is remarkable the very good agreement between the temperatures
obtained from fitting the anti-Stokes emission and the water temperatures
measured by the Pt$100$ thermometer. The calibration-free procedure allows to
perform the same measurements in any other setup and could act as a reference
for calibration of other nano thermometers.
...
section given by Mie theory. 

As explained earlier for nanorods, the fitting of the anti-Stokes emission is
highly dependent on the plasmon position and the laser employed. In the case of
nanospheres, the resonance is slightly shifted to the red from the $532\nm$
laser. Small errors in the determination of the plasmon position will
generate larger errors in the temperature extracted. The discrepancy between the
plasmon observed in fig. \ref{fig:spheres} and the results of Mie scattering
calculations most likely are due to particles that are not perfectly spherical.
If this was the case, the calculated cross section and plasmon resonance will
not coincide with the particles under study, leading to systematic errors.
